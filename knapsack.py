...
def plotGeneticAlgorithm(highestFitness, averageFitness, populationSize, mutationRate):
    print(len(averageFitness), len(highestFitness))
    plt.plot(highestFitness, label="Highest")
    plt.plot(averageFitness, label="Average")
    plt.ylabel('Value')
    plt.xlabel('Generation')
    plt.legend(loc="upper left")

    plt.gcf().text(0.05, 0.9, "Mutation rate: {}\nGeneration size: {}".format(mutationRate, populationSize), fontsize=9)

    plt.gcf().text(0.5, 0.9, "Best solution found: {:5.5}".format(max(highestFitness)), fontsize=11)
...
class GeneticAlgorithm:
    ...
    def run(self, generations, cutoff_percent):
        ...
        for gen_id in range(generations):
            ...
            avg, highest = self.generation_report(gen_id, mutated_children, self.pop_fitness)
            average_fitnesses.append(avg)
            max_fitnesses.append(highest)

            # if (gen_id >= 10):
            #     average_fitnesses.pop(0)
            if (self.average_across_generations(average_fitnesses) < cutoff_percent and gen_id > 100):
                break
        print(len(max_fitnesses), len(average_fitnesses))
        plotGeneticAlgorithm(max_fitnesses, average_fitnesses, self.populationSize, self.mutationRate)
...
if __name__ == '__main__':
    ...
    GeneticAlgorithm("Input-Items.txt", 400, 500, 0.0001, 0.001, 1000, 1000)
    print('Complete')
